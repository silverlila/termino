...
- Shared test scaffolding (ephemeral-port relay, temp-dir cleanup, `wait`) lives in `test/support/harness.ts`; shared TUI mounting lives in `test/support/tui.tsx`. The rest of `test/` mirrors `src/` file for file, so a new helper that mirrors nothing belongs under `test/support/`.
- `test/crypto/identity.test.ts`'s "concurrent first runs" case spawns 8 `bun -e` subprocesses synchronised on a 500 ms wall-clock barrier. Under full-suite load a racer can miss the window and fail spuriously — seen roughly once in ten full runs, never in isolation. Re-run before investigating.
