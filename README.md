...
```
src/crypto/     derivation, AEAD, identity, fingerprints
src/protocol/   the wire contract: outer frames, inner signed payloads
src/client/     headless session, trust store, and the TUI on top of them
server/relay.ts the relay — a separate deployable, which imports the wire
